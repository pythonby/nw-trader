...
def run_backtest_signal_only(df: pd.DataFrame, signal_type: str, holding_bars: int = 10):
    """
    Pure signal backtest — analyzes EVERY occurrence of a signal independently,
    not waiting for an opposite signal to close.

    signal_type: 'BUY' (lower band touches) or 'SELL' (upper band touches)
    holding_bars: how many bars forward to measure the outcome (default 10)

    For BUY: measures price move UP after lower band touch (mean reversion)
    For SELL: measures price move DOWN after upper band touch (mean reversion)
    """
    trades = []
    ...
    for i in range(n):
        ...
        if sig in target_signals:
            ...
            # Look forward up to holding_bars to find exit price
            ...
            if signal_type == 'BUY':
                # Expect price to go UP from lower band
                pnl_pct = (exit_price - entry_price) / entry_price * 100
            else:
                # Expect price to go DOWN from upper band (short logic)
                pnl_pct = (entry_price - exit_price) / entry_price * 100

            # Also track max favorable move within holding period
            future_slice = df['Close'].iloc[i:exit_idx+1]
            if signal_type == 'BUY':
                best_price = future_slice.max()
                best_pct = (best_price - entry_price) / entry_price * 100
            else:
                best_price = future_slice.min()
                best_pct = (entry_price - best_price) / entry_price * 100

            trades.append({
                'Signal Time':   entry_time,
                'Signal Price':  round(float(entry_price), 4),
                f'Exit (+{holding_bars} bars)': round(float(exit_price), 4),
                'Exit Time':     exit_time,
                'PnL %':         round(pnl_pct, 2),
...
with tab_backtest:
    ...
    if bt_mode != "🔁 Full Strategy (BUY→SELL pairs)":
        st.caption("📊 Har signal occurrence ko alag analyze karega — aage N bars mein price kitna move hui, wo dikhayega")
        holding_bars = st.slider("📏 Kitne bars aage result check karein", 3, 50, 10, 1,
                                  help="Signal aane ke baad kitne candles aage dekh ke result nikalna hai")
    else:
        holding_bars = 10
            ...
            if df_bt.empty:
                ...
                if len(df_bt) < 30:
                    ...
                    # ══════════════════════════════════════
                        ...
                        trades_df, stats = run_backtest_signal_only(df_bt, sig_type, holding_bars)
