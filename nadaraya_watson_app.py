...
with tab_alerts:
    ...
    if 'tg_token'   not in st.session_state: st.session_state.tg_token   = _PRESET_TOKEN
    if 'tg_chat_id' not in st.session_state: st.session_state.tg_chat_id = _PRESET_CHAT_ID

    tg_c1, tg_c2 = st.columns(2)
    with tg_c1:
        tg_token_input = st.text_input("🤖 Bot Token", value=st.session_state.tg_token,
            placeholder="7123456789:AAGxxxxx", type="password", key="tg_tok_inp")
    with tg_c2:
        tg_chat_input = st.text_input("💬 Chat ID", value=st.session_state.tg_chat_id,
            placeholder="987654321", key="tg_cid_inp")

    btn1, btn2, _ = st.columns([1,1,2])
    with btn1:
        if st.button("💾 Save", use_container_width=True, type="primary", key="tg_save_btn"):
    ...
    with btn2:
        if st.button("📨 Test", use_container_width=True, key="tg_test_btn"):
            ...
            if not tok: st.error("❌ Bot Token missing!")
            elif not cid: st.error("❌ Chat ID missing!")
            else:
                try:
                    ...
                    elif r.status_code == 401: st.error("❌ Token galat hai!")
                    else:
                        err = r.json().get("description","")
                        st.error(f"❌ {err if err else r.text[:100]}")
    ...
    if _PRESET_TOKEN: st.caption("🔐 Token Streamlit Secrets se load hua — secure!")
    if tg_ok:
    ...
    # ══════════════════════════════════════════════
    # SAVED ALERT RULES MANAGER
    # ══════════════════════════════════════════════
    st.markdown("#### 💾 Saved Alert Rules")
    st.caption("Multiple alag-alag alerts save karo — har ek ka naam, condition, symbols alag ho sakta hai")

    # Initialize saved alerts
    if 'saved_alerts' not in st.session_state:
        st.session_state.saved_alerts = {}

    # Show all saved alerts
    if st.session_state.saved_alerts:
        for alert_name, alert_config in list(st.session_state.saved_alerts.items()):
            is_on = alert_config.get("active", False)
            bg = "#0d2818" if is_on else "#1c2128"
            ...
            <div style='background:{bg}; border:1px solid {border}; border-radius:8px;
                ...
                <b style='color:#e6edf3'>{alert_name}</b>
                <span style='float:right; color:{"#3fb950" if is_on else "#8b949e"}'>{status_icon}</span><br>
                <small style='color:#8b949e'>
                📌 {alert_config.get("symbols_desc","—")} &nbsp;|&nbsp;
                ⏱ {alert_config.get("timeframes_desc","—")} &nbsp;|&nbsp;
                🎯 {alert_config.get("signal_type","BUY+SELL")}
                </small>
            </div>
            """, unsafe_allow_html=True)
            ...
            with ac1:
                ...
                btn_label = "⏸ Pause" if is_on else "▶ Activate"
                if st.button(btn_label, key=f"al_toggle_{alert_name}", use_container_width=True):
                    st.session_state.saved_alerts[alert_name]["active"] = new_status
                    st.rerun()
            with ac2:
                if st.button("🔍 Check Now", key=f"al_check_{alert_name}", use_container_width=True):
                    st.session_state['run_alert_name'] = alert_name
            with ac3:
                if st.button("✏️ Edit", key=f"al_edit_{alert_name}", use_container_width=True):
                    st.session_state['edit_alert_name'] = alert_name
            with ac4:
                if st.button("🗑 Delete", key=f"al_del_{alert_name}", use_container_width=True):
                    del st.session_state.saved_alerts[alert_name]
                    st.rerun()
    else:
        st.info("Koi saved alert nahi — neeche nayi alert banao! ⬇️")
    ...
    # CREATE / EDIT ALERT
    # ══════════════════════════════════════════════
    edit_mode = 'edit_alert_name' in st.session_state and st.session_state.get('edit_alert_name','') in st.session_state.saved_alerts
    edit_name = st.session_state.get('edit_alert_name','') if edit_mode else ''
    edit_cfg  = st.session_state.saved_alerts.get(edit_name, {}) if edit_mode else {}

    st.markdown(f"#### {'✏️ Alert Edit Karo' if edit_mode else '➕ Nayi Alert Banao'}")

    # Alert name
    al_name_default = edit_name if edit_mode else f"Alert {len(st.session_state.saved_alerts)+1}"
    alert_name_input = st.text_input("🏷️ Alert ka naam", value=al_name_default,
        placeholder="e.g. Nifty BUY Alert, Bank Lower Band...", key="al_name_inp")

    # Alert source
    al_src = st.radio("📂 Symbols",
        ["📌 Particular Stock", "🏭 Sector/Category", "📋 Watchlist"],
        horizontal=True, key="al_src_radio",
        index=["📌 Particular Stock","🏭 Sector/Category","📋 Watchlist"].index(
            edit_cfg.get("source","📌 Particular Stock")) if edit_mode else 0)

    alert_symbols = []
    symbols_desc  = ""

    if al_src == "📌 Particular Stock":
        ps_c1, ps_c2 = st.columns(2)
        with ps_c1:
            ps_cat = st.selectbox("Category", list(INDEX_DATABASE.keys()), key="al_ps_cat")
        with ps_c2:
            ps_opts = {k:v for k,v in INDEX_DATABASE[ps_cat].items() if v!="CUSTOM"}
            default_sym = edit_cfg.get("symbols_desc","").split(",")[0].strip()
            ps_label = st.selectbox("Stock", list(ps_opts.keys()), key="al_ps_sym")
        alert_symbols = [ps_opts[ps_label]]
        symbols_desc  = ps_opts[ps_label]

        # Extra custom stocks
        if 'al_extra_stocks' not in st.session_state:
            st.session_state.al_extra_stocks = edit_cfg.get("extra_stocks",[])
        ex_c1, ex_c2 = st.columns([3,1])
        with ex_c1:
            new_al_stock = st.text_input("➕ Aur stocks (optional)",
                placeholder="ZOMATO.NS, WIPRO.NS", key="al_extra_inp", label_visibility="collapsed")
        with ex_c2:
            if st.button("➕ Add", key="al_add_extra"):
                s_add = new_al_stock.strip().upper()
                if s_add and s_add not in st.session_state.al_extra_stocks:
                    st.session_state.al_extra_stocks.append(s_add)
                    st.rerun()
        for i_e, se in enumerate(list(st.session_state.al_extra_stocks)):
            ec1, ec2 = st.columns([4,1])
            ec1.caption(f"`{se}`")
            if ec2.button("❌", key=f"al_rem_ex_{se}_{i_e}"):
                st.session_state.al_extra_stocks.remove(se)
                st.rerun()
        alert_symbols += st.session_state.al_extra_stocks
        alert_symbols  = list(dict.fromkeys(alert_symbols))
        symbols_desc   = ", ".join(alert_symbols[:3]) + ("..." if len(alert_symbols)>3 else "")

    elif al_src == "🏭 Sector/Category":
        sec_opts = ["⭐ ALL"] + list(INDEX_DATABASE.keys())
        al_sector = st.selectbox("Sector", sec_opts, key="al_sector_sel")
        if al_sector == "⭐ ALL":
            for v in INDEX_DATABASE.values():
                alert_symbols += [t for t in v.values() if t!="CUSTOM"]
        else:
            alert_symbols = [t for t in INDEX_DATABASE.get(al_sector,{}).values() if t!="CUSTOM"]
        alert_symbols = list(dict.fromkeys(alert_symbols))
        symbols_desc  = al_sector

    else:
        if 'watchlists' in st.session_state and st.session_state.watchlists:
            al_wl = st.selectbox("Watchlist", list(st.session_state.watchlists.keys()), key="al_wl_sel")
            alert_symbols = st.session_state.watchlists.get(al_wl, [])
            symbols_desc  = f"Watchlist: {al_wl}"
        else:
            st.warning("Koi watchlist nahi!")

    st.caption(f"📊 {len(alert_symbols)} symbols selected")

    # Timeframe rules
    st.markdown("**⏱ Timeframe Rules:**")

    if 'al_rules' not in st.session_state:
        st.session_state.al_rules = edit_cfg.get("rules", [
            {"tf": "15m", "buy": True,  "sell": True,  "active": True,  "_id": 0},
            {"tf": "1H",  "buy": True,  "sell": True,  "active": True,  "_id": 1},
            {"tf": "4H",  "buy": False, "sell": False, "active": False, "_id": 2},
        ])

    h1,h2,h3,h4 = st.columns([1.5,1,1,1])
    h1.markdown("**⏱ TF**"); h2.markdown("**✅ BUY**"); h3.markdown("**🔴 SELL**"); h4.markdown("**ON**")

    for i_r, rule in enumerate(st.session_state.al_rules):
        rid = rule.get("_id", i_r)
        ...
        with rc[0]:
            tf_s = st.selectbox("", list(TIMEFRAMES.keys()),
                ...
                key=f"al_rtf_{rid}", label_visibility="collapsed")
            st.session_state.al_rules[i_r]["tf"] = tf_s
        with rc[1]:
            buy_s = st.checkbox("BUY", value=rule["buy"], key=f"al_rbuy_{rid}", label_visibility="collapsed")
            st.session_state.al_rules[i_r]["buy"] = buy_s
        with rc[2]:
            sell_s = st.checkbox("SELL", value=rule["sell"], key=f"al_rsell_{rid}", label_visibility="collapsed")
            st.session_state.al_rules[i_r]["sell"] = sell_s
        with rc[3]:
            act_s = st.checkbox("ON", value=rule["active"], key=f"al_ract_{rid}", label_visibility="collapsed")
            st.session_state.al_rules[i_r]["active"] = act_s
        if "_id" not in rule: st.session_state.al_rules[i_r]["_id"] = i_r

    ra1, ra2, _ = st.columns([1,1,2])
    with ra1:
        if st.button("➕ Row", key="al_add_row") and len(st.session_state.al_rules) < 8:
            new_id = max([r.get("_id",0) for r in st.session_state.al_rules], default=0) + 1
            st.session_state.al_rules.append({"tf":"1D","buy":True,"sell":True,"active":True,"_id":new_id})
            st.rerun()
    with ra2:
        if st.button("➖ Row", key="al_rem_row") and len(st.session_state.al_rules) > 1:
            st.session_state.al_rules.pop()
            st.rerun()

    active_al_rules = [r for r in st.session_state.al_rules if r["active"]]
    tf_desc = " | ".join([f"{'✅' if r['buy'] else ''}{'🔴' if r['sell'] else ''}{r['tf']}" for r in active_al_rules])
    signal_type_desc = "BUY+SELL" if any(r['buy'] for r in active_al_rules) and any(r['sell'] for r in active_al_rules) else                        "BUY Only" if any(r['buy'] for r in active_al_rules) else "SELL Only"

    # Save Alert button
    st.write("")
    sv1, sv2, sv3 = st.columns([1,1,1])
    with sv1:
        if st.button("💾 Save Alert", use_container_width=True, type="primary", key="al_save_btn"):
            name = alert_name_input.strip()
            if not name:
                st.error("Alert ka naam likhein!")
            elif not alert_symbols:
                st.error("Koi symbol nahi!")
            elif not active_al_rules:
                st.error("Koi active rule nahi!")
            else:
                st.session_state.saved_alerts[name] = {
                    "active":        True,
                    "source":        al_src,
                    "symbols":       alert_symbols,
                    "symbols_desc":  symbols_desc,
                    "rules":         st.session_state.al_rules.copy(),
                    "timeframes_desc": tf_desc,
                    "signal_type":   signal_type_desc,
                    "extra_stocks":  st.session_state.get('al_extra_stocks',[]),
                }
                if edit_mode and edit_name != name:
                    del st.session_state.saved_alerts[edit_name]
                if 'edit_alert_name' in st.session_state:
                    del st.session_state['edit_alert_name']
                st.success(f"✅ '{name}' saved!")
                st.rerun()
    with sv2:
        if st.button("🔄 Reset Form", use_container_width=True, key="al_reset_btn"):
            if 'edit_alert_name' in st.session_state: del st.session_state['edit_alert_name']
            if 'al_extra_stocks' in st.session_state: st.session_state.al_extra_stocks = []
            st.session_state.al_rules = [
                {"tf":"15m","buy":True, "sell":True, "active":True,"_id":0},
                {"tf":"1H", "buy":True, "sell":True, "active":True,"_id":1},
            ]
            st.rerun()
    with sv3:
        if edit_mode:
            if st.button("❌ Cancel Edit", use_container_width=True, key="al_cancel_btn"):
                del st.session_state['edit_alert_name']
                st.rerun()
    ...
    # RUN ALERTS
    # ══════════════════════════════════════════════
    st.markdown("#### 🚀 Alerts Run Karo")

    run_mode = st.radio("Kaunsa run karna hai?",
        ["✅ Sirf Active Alerts (sab ek saath)", "📌 Ek specific alert select karo", "🔍 Manual Check (naya)"],
        horizontal=True, key="al_run_mode")

    if run_mode == "📌 Ek specific alert select karo" and st.session_state.saved_alerts:
        sel_alert = st.selectbox("Alert select karo",
            list(st.session_state.saved_alerts.keys()), key="al_sel_run")
    else:
        sel_alert = None

    check_alerts = st.button("🔍 Check Now", use_container_width=False, type="primary", key="al_check_now_btn")
    ...
    def run_alert_check(check_syms, rules, alert_label=""):
        """Run alert check for a set of symbols and rules"""
        results = []
        active_rules = [r for r in rules if r.get("active", False)]
        if not active_rules or not check_syms: return results

        total = len(active_rules) * len(check_syms)
        prog = st.progress(0)
        stat = st.empty()
        count = 0
        ...
        for rule in active_rules:
            ...
            for sym_al in check_syms:
                stat.text(f"⏳ {alert_label} | {sym_al} | {tf_r} ({count+1}/{total})")
                ...
                if not df_al.empty and len(df_al) > 50:
                    ...
                    up_v     = float(last_al['upper']) if not np.isnan(last_al['upper']) else 0
                    lo_v     = float(last_al['lower']) if not np.isnan(last_al['lower']) else 0
                    ts_al    = str(df_al.index[-1])[:16]
                    dist_up  = round((up_v - price_al)/price_al*100, 2) if up_v else 999
                    dist_lo  = round((price_al - lo_v)/price_al*100, 2) if lo_v else 999
                    near_upper = dist_up <= 0.5 or price_al >= up_v
                    near_lower = dist_lo <= 0.5 or price_al <= lo_v
                    if near_upper and rule.get("sell", False):
                        results.append({"type":"SELL","symbol":sym_al,"price":price_al,
                            "time":ts_al,"tf":tf_r,"dist":f"{dist_up}% from upper","alert":alert_label,
                            "msg":f"📉 <b>NW Band Scanner</b>\n━━━━━━━━━━━━━\n🔴 <b>SELL</b>\n📌 {sym_al}\n⏱ {tf_r}\n💰 ₹{price_al:,}\n📊 Upper Band ({dist_up}%)\n🕐 {ts_al}\n🏷 {alert_label}\n━━━━━━━━━━━━━"})
                    elif near_lower and rule.get("buy", False):
                        results.append({"type":"BUY","symbol":sym_al,"price":price_al,
                            "time":ts_al,"tf":tf_r,"dist":f"{dist_lo}% from lower","alert":alert_label,
                            "msg":f"📈 <b>NW Band Scanner</b>\n━━━━━━━━━━━━━\n✅ <b>BUY</b>\n📌 {sym_al}\n⏱ {tf_r}\n💰 ₹{price_al:,}\n📊 Lower Band ({dist_lo}%)\n🕐 {ts_al}\n🏷 {alert_label}\n━━━━━━━━━━━━━"})
                count += 1
    ...
    if check_alerts:
        ...
        if run_mode == "✅ Sirf Active Alerts (sab ek saath)":
            active_saved = {k:v for k,v in st.session_state.saved_alerts.items() if v.get("active",False)}
            if not active_saved:
                st.warning("⚠️ Koi active saved alert nahi! Upar banao aur activate karo.")
            else:
                for al_name, al_cfg in active_saved.items():
                    found = run_alert_check(al_cfg["symbols"], al_cfg["rules"], al_name)
                    all_alerts.extend(found)

        elif run_mode == "📌 Ek specific alert select karo" and sel_alert:
            al_cfg = st.session_state.saved_alerts[sel_alert]
            all_alerts = run_alert_check(al_cfg["symbols"], al_cfg["rules"], sel_alert)

        else:
            # Manual mode — use current form state
            if not alert_symbols: st.error("❌ Koi symbol nahi!")
            else: all_alerts = run_alert_check(alert_symbols, st.session_state.al_rules, "Manual Check")

        if all_alerts:
            st.markdown(f"#### 🔔 {len(all_alerts)} Alert(s) Found!")
            for al in all_alerts:
                if al['type'] == 'BUY':
                    st.markdown(f'<div class="alert-buy">✅ <b>BUY</b> — {al["symbol"]} | ⏱ {al["tf"]} | 💰 ₹{al["price"]:,} | 📊 {al["dist"]} | 🏷 {al["alert"]}</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="alert-sell">🔴 <b>SELL</b> — {al["symbol"]} | ⏱ {al["tf"]} | 💰 ₹{al["price"]:,} | 📊 {al["dist"]} | 🏷 {al["alert"]}</div>', unsafe_allow_html=True)
                if tg_ok:
                    ok, err = send_tg(st.session_state.tg_token, st.session_state.tg_chat_id, al['msg'])
                    if ok: st.caption(f"  📱 Sent: {al['symbol']} {al['tf']}")
                    else: st.warning(f"  ⚠️ TG failed: {err}")
    ...
    if st.session_state.alert_log:
        ...
        log_df = pd.DataFrame([{
            'Time':   a['time'],   'Alert':  a.get('alert',''),
            'Type':   a['type'],   'Symbol': a['symbol'],
            'TF':     a.get('tf',''), 'Price': f"₹{a['price']:,}",
            'Band':   a.get('dist',''),
        ...
        with cl1:
            if st.button("🗑 Clear History", key="al_clear_hist"):
