...
with tab_alerts:
    ...
    with st.expander("📱 Telegram Setup & Config", expanded=False):
        st.markdown("""
        **Step 1** → Telegram mein **@BotFather** → `/newbot` → Token copy karo
        **Step 2** → **@userinfobot** → `/start` → Chat ID copy karo
        **Step 3** → Neeche Token + Chat ID save karo → Test karo
        """)
        ...
        with tc1:
            tg_tok = st.text_input("🤖 Bot Token", value=st.session_state.tg_token,
                type="password", placeholder="7123456789:AAGxxxxx", key="tg_tok_inp2")
        with tc2:
            tg_cid = st.text_input("💬 Chat ID", value=st.session_state.tg_chat_id,
                placeholder="987654321", key="tg_cid_inp2")
        tb1, tb2, _ = st.columns([1,1,2])
        with tb1:
            if st.button("💾 Save Config", key="tg_save2"):
                st.session_state.tg_token   = tg_tok.strip()
                st.session_state.tg_chat_id = tg_cid.strip()
                st.success("✅ Saved!")
        with tb2:
            if st.button("📨 Test", key="tg_test2"):
                tok = st.session_state.tg_token; cid = st.session_state.tg_chat_id
                if tok and cid:
                    try:
                        ...
                        elif r.status_code==401: st.error("❌ Token galat!")
                        else: st.error(f"❌ {r.json().get('description','Error')}")
