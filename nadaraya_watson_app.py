...
def init_persistent_state():
    """
    Initialize all persistent state on first load.
    Token comes from Streamlit Secrets (permanent).
    Saved alerts are restored from st.session_state (per-session).
    """
    # ── Token: Always load from secrets if available ──
    if "tg_token" not in st.session_state or not st.session_state.tg_token:
        st.session_state.tg_token = _PRESET_TOKEN
    if "tg_chat_id" not in st.session_state or not st.session_state.tg_chat_id:
        st.session_state.tg_chat_id = _PRESET_CHAT_ID

    # ── If token from secrets exists, always override session ──
    # This fixes "revoked token" issue — secrets token always wins
    if _PRESET_TOKEN:
        st.session_state.tg_token   = _PRESET_TOKEN
    if _PRESET_CHAT_ID:
        st.session_state.tg_chat_id = _PRESET_CHAT_ID

    # ── Saved Alerts: Initialize empty if not exists ──
...
with tab_alerts:
    ...
    with st.expander("📱 Telegram Setup & Config", expanded=False):
        ...
        **Token kahan se milega:**
        ...
        with tb1:
            if st.button("💾 Session Save", key="tg_save2",
                help="Sirf is session ke liye save — app restart hone par Secrets wala token aayega"):
                st.session_state.tg_token   = tg_tok.strip()
                st.session_state.tg_chat_id = tg_cid.strip()
                st.success("✅ Session mein saved! (Permanent ke liye Secrets use karo)")
        with tb2:
        ...
        for al_name, al_cfg in list(st.session_state.saved_alerts.items()):
            ...
            with ac1:
                if st.button("⏸ Pause" if is_on else "▶ Activate", key=f"tog_{al_name}", use_container_width=True):
                    st.session_state.saved_alerts[al_name]["active"] = not is_on; st.rerun()
            with ac2:
            ...
            with ac3:
                if st.button("🗑 Delete", key=f"del_{al_name}", use_container_width=True):
                    del st.session_state.saved_alerts[al_name]; st.rerun()
    else:
    ...
    with s1c1:
        al_cat3 = st.selectbox("Category", list(INDEX_DATABASE.keys()), key="al_cat3")
    with s1c2:
    ...
    st.caption(f"✅ {len(final_syms3)} symbol(s): {', '.join(final_syms3[:3])}{'...' if len(final_syms3)>3 else ''}")
    ...
    if st.button("💾 Alert Save Karo", type="primary", use_container_width=True, key="al_save3"):
        if not al_name_inp.strip(): st.error("❌ Alert naam likhein!")
            ...
            st.success(f"✅ Alert '{al_name_inp}' saved!")
            st.rerun()
